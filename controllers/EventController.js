...
async inscription (req, res) {
  try {
    const event = await Event.findOne({where:{id:req.body.compId}})
    const user = await User.findOne({where:{id:req.body.userId}})
      await event.addUser(user,{ through: { catId: req.body.catId } })
      const EventJson = event.toJSON()
      res.send({
  ...
  async in (req, res) {
    ...
    async elig (req, res) {
      try {
        const user = await User.findOne({where:{id:req.body.userId}})
        const cat = await Category.findOne({where:{id:req.body.catId}})
       if(eligeable(cat.weight,user.weight,cat.sexe,user.sexe,user.matricule)==true)
      {console.log("leig")
        catId=cat.id
        res.status(200).send({message:"ok"})
       }
       else{
        res.status(201).send({message:"not ok"})
       }
          }catch (error) {
            res.status(400).send({
              error: error.message
